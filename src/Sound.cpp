...
static void InitAudioDev()
{
	...
	// CoreAudio is initialised by CoreAudioStreamer (in AppDelegate).
	// Sound buffer already set up by MacPlatformInit.
	SoundEnabled = true;
	...
	if (!SoundSamplesLoaded)
	{
		for (int i = 0; i < NUM_SOUND_SAMPLES; ++i)
		{
			...
			if (fd != nullptr)
			{
				...
				{
					free(SoundSamples[i].pBuf);
					SoundSamples[i].pBuf = nullptr;
					SoundSamples[i].len = 0;
				}
			}
			// Missing .snd files are non-fatal — disc drive sounds simply won't play.
		}
...
void SoundReset()
{
	...
	// CoreAudio teardown is handled by CoreAudioStreamer.
