...
BeebWin::BeebWin()
{
	...
	strcpy(m_AppPath, GetBundleResourcesPath());
	...
	AppendPath(m_UserDataPath, "Library");
	AppendPath(m_UserDataPath, "Application Support");
	AppendPath(m_UserDataPath, "BeebEm");
...
bool BeebWin::Initialise()
{
	...
	{
		// Non-fatal: Mode 7 text will be blank; log only, no dialog.
		fprintf(stderr, "BeebEm: cannot open Teletext font: %s\n", FontFilename);
	}
...
{
	CheckMenuItem(IDM_REALTIME,       !m_FPSTarget && m_TimingSpeed == 100);
CheckMenuItem(IDM_50FPS,          m_FPSTarget == 50);
	CheckMenuItem(IDM_25FPS,          m_FPSTarget == 25);
	CheckMenuItem(IDM_10FPS,          m_FPSTarget == 10);
	CheckMenuItem(IDM_5FPS,           m_FPSTarget == 5);
	CheckMenuItem(IDM_1FPS,           m_FPSTarget == 1);
	...
	CheckMenuItem(IDM_FIXEDSPEED1,   m_TimingType == TimingType::FixedSpeed && m_TimingSpeed == 100);
...
void BeebWin::EditRomConfig()
{
	...
	if (MacEditRomConfig(RomConfig, m_UserDataPath))
	{
		BeebReadRoms();
	}
...
void BeebWin::OpenUserKeyboardDialog()
{
	...
	if (!UserKeyboardDialog(m_hWnd))
	{
		// Dialog not shown (non-WIN32 stub) — resume immediately.
		if (!m_WasPaused)
		{
			TogglePause();
		}
...
{
	#ifdef __APPLE__
	Report(MessageType::Info, "The debugger is not available in this macOS build.");
	#else
...
MessageResult BeebWin::ReportV(MessageType type, const char *format, va_list args)
{
	...
	switch (type)
	{
		case MessageType::Question: macType = 2; break;
		case MessageType::Confirm:  macType = 3; break;
		case MessageType::Info:
		case MessageType::Warning:  macType = 1; break;
		default:                    macType = 0; break;
	}

	int ret = MacReport(macType, WindowTitle, pszMessage);

	switch (type)
	{
		case MessageType::Question:
			Result = (ret == 1) ? MessageResult::Yes : MessageResult::No;
			break;
		case MessageType::Confirm:
			Result = (ret == 1) ? MessageResult::OK : MessageResult::Cancel;
			break;
		default:
			Result = MessageResult::OK;
			break;
