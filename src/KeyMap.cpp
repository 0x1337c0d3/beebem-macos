	...
	else
	{
		...
		else
		{
			...
			for (int i = 0; i < KEYMAP_SIZE; ++i)
			{
				...
				{
					// File has fewer entries than KEYMAP_SIZE — leave remainder unbound.
					break;
				}
...
// On macOS keyboard events use Cocoa VK codes, not SDL keysyms.
// Return nullptr so WriteKeyMap omits the symbolic label in keymap files.
static const char* GetVirtualKeyCode(int /*Key*/)
{
	return nullptr;
}
